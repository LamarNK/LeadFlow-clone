...
class WorkerMsiContractTests(unittest.TestCase):
    ...
    def test_generated_wxs_launches_with_start_and_update_restart(self) -> None:
        ...
        self.assertIn('Shortcut Id="OrbitaWorkerDesktopShortcut"', text)
        self.assertIn('Directory="DesktopFolder"', text)
        self.assertIn('Target="[INSTALLFOLDER]Orbita.Worker.exe"', text)
        self.assertIn('ComponentRef Id="DesktopShortcutComponent"', text)
