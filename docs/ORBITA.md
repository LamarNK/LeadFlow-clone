...
- в строке аккаунта есть ⚙ **Настройки профиля** (компактная боковая панель, только для Local Chrome): логин/пароль Avito, HTTP-прокси `host:port`, блок **Скорость и трафик** и кнопка «Открыть браузер»;
- **Скорость и трафик** есть только у обычного Chrome. AdsPower и Multilogin этот блок не показывают и поля не хранят. Пресеты: Обычный (ничего не блокирует, timeout 60 с), Экономный (видео/аудио, внешняя аналитика, предзагрузка), Агрессивный (экономный + изображения карточек и веб-шрифты). Если переключатели не совпадают с пресетом, появляется «Пользовательский»;
- блокировки действуют только в мониторинговой сессии Local Chrome через request interception на рабочей вкладке. `document` / `script` / `xhr` / `fetch` / `stylesheet` / WebSocket не режутся; аналитика — только явный список доменов (`mc.yandex.ru`, `top.mail.ru`, `google-analytics.com`, `googletagmanager.com`, `doubleclick.net`); prefetch — только заголовок `Purpose`/`Sec-Purpose: prefetch`. При капче изображения временно разрешаются. «Открыть браузер» вручную ничего не блокирует;
- кэш профиля включён всегда. В панели показывается компактная статистика последнего мониторинга (длительность первой навигации и счётчики блокировок) без URL запросов;
