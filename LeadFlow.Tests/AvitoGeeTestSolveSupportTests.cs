...
public sealed class AvitoGeeTestSolveSupportTests
{
    ...
    public void LoginClickCaptcha_CapturesNineGridFromVisibleCssComposition()
    {
        ...
        Assert.Contains("prepared.IsNineGrid", source, StringComparison.Ordinal);
        Assert.Contains("? LoginNineGridCaptchaImageSelector", source, StringComparison.Ordinal);
        Assert.Contains("LoginClickCaptchaHintSelector", source, StringComparison.Ordinal);
        ...
        Assert.Contains("hintUrls.length", source, StringComparison.Ordinal);
        ...
        Assert.Contains("var hintImageBody = await preparedHint.ScreenshotBase64Async()", source, StringComparison.Ordinal);
        Assert.DoesNotContain("document.createElement('canvas')", source, StringComparison.Ordinal);
    }
